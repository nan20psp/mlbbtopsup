...
def load_global_settings():
    ...
    """
    global g_settings
    g_settings = db.load_settings(DEFAULT_PAYMENT_INFO, DEFAULT_MAINTENANCE)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    load_authorized_users() # Reload global set from DB
    ...
    if not user_doc:
        db.create_user(user_id, name, username)
    ...
    msg = (
        ...
        "💎 ***𝗦𝗔𝗦𝗨𝗞𝗘 𝗠𝗟𝗕𝗕 𝗧𝗢𝗣 𝗨𝗣 𝗕𝗢𝗧*** မှ ကြိုဆိုပါတယ်။\n\n"
...
async def c_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id in user_states and user_states[user_id] == "waiting_approval":
        await update.message.reply_text(
            ...
            "🔒 ***Screenshot ပို့ပြီးပါပြီ။ Admin approve စောင့်ပါ။***",
            parse_mode="Markdown"
        )
        return
    ...
    if not re.match(pattern, expression):
        await update.message.reply_text("❌ ***မှားယွင်းသော expression!***")
        return

    if not any(op in expression for op in ['+', '-', '*', '/']):
        await update.message.reply_text("❌ ***Operator မရှိပါ!***")
        return
    ...
    try:
        ...
        await update.message.reply_text(f"❌ မှားယွင်းသော expression!: {e}")
...
async def history_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    topups = db.get_user_topups(user_id, limit=5)
...
async def setprice_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(args) != 2:
        # We already checked for len < 2 at the start, so this catches cases like
        # /setprice normal 1 2 3 (which is handled above)
        # but also catches /setprice singleitem 1000 2000 (which is invalid)
        await update.message.reply_text(
            "❌ ***Format မှားနေပါသည်!***\n\n"
...
async def sasukemlbbtopup_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # --- Command List ---
    # main.py ထဲတွင် register လုပ်ထားသော command များ
    command_list_text = """
...
---
`/approve` - (user_id amount) - Topup လက်ခံရန်
`/deduct` - (user_id amount) - Balance နှုတ်ရန်
`/reply` - (user_id message) - User ထံ reply ပြန်ရန်
`/done` - (user_id) - "Order Done" message ပို့ရန်
`/ban` - (user_id) - User ကို ban ရန်
`/unban` - (user_id) - User ကို unban ရန်
...
---
`/addadm` - (user_id) - Admin အသစ်ခန့်ရန်
`/unadm` - (user_id) - Admin ဖြုတ်ရန်
...
`/clearhistory` - (user_id) - User history ဖျက်ရန်
`/cleanmongodb` - (confirm) - **[DANGER]** DB တစ်ခုလုံး ဖျက်ရန်

---
🤖 **Clone Bot Commands** (Admin/Owner)
---
`/addbot` - (token) - Bot အသစ် ထည့်ရန် (Admin)
`/listbots` - Bot list ကြည့်ရန် (Admin)
`/removebot` - (bot_id) - Bot ဖြုတ်ရန် (Owner)
`/addfund` - (admin_id amount) - Clone bot balance ဖြည့်ရန် (Owner)
`/deductfund` - (admin_id amount) - Clone bot balance နှုတ်ရန် (Owner)
...
def load_clone_bots():
    """Load clone bots from DB"""
    return db.load_clone_bots()

def save_clone_bot(bot_id, bot_data):
    """Save clone bot to DB"""
    db.save_clone_bot(bot_id, bot_data)

def remove_clone_bot(bot_id):
    """Remove clone bot from DB"""
    return db.remove_clone_bot(bot_id)


# main.py (Function အသစ် ထပ်ထည့်ရန်)

async def clonetopupbot_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    User-facing command to request a new clone bot.
    Usage: /clonetopupbot <bot_token>
    """
    user_id = str(update.effective_user.id)
    user_name = f"{update.effective_user.first_name} {update.effective_user.last_name or ''}".strip()
    
    # 1. Must be an authorized user
    if not is_user_authorized(user_id):
        await update.message.reply_text("🚫 Bot ကို အသုံးပြုခွင့်ရရန် /start နှိပ်ပြီး Register လုပ်ပါ။")
        return
    ...
    if len(args) != 1:
        await update.message.reply_text(
            "❌ **Format မှားနေပါသည်!**\n\n"
            "သင့် bot token ကို @BotFather မှ copy ကူးပြီး အောက်ပါအတိုင်း ရိုက်ထည့်ပါ:\n"
            "`/clonetopupbot 123456:ABC-DEF...`",
            parse_mode="Markdown"
        )
        return
    ...
    # 2. Validate token
    try:
        processing_msg = await update.message.reply_text("⏳ Token ကို စစ်ဆေးနေပါသည်...")
        temp_bot = Bot(token=bot_token)
        bot_info = await temp_bot.get_me()
        bot_username = bot_info.username
        bot_id = str(bot_info.id)
    except Exception as e:
        await update.message.reply_text(f"❌ Bot token မှားနေပါတယ်!\nError: {str(e)}")
        return
        
    # 3. Check if bot already exists (active or pending)
    if db.get_clone_bot(bot_id):
        await processing_msg.edit_text(f"ℹ️ ဒီ bot (@{bot_username}) က စာရင်းသွင်းပြီးသားပါ!")
        return
    ...
    bot_data = {
        ...
        "owner_id": user_id, # The requester is the owner
        "balance": 0,
        "status": "pending", # <--- PENDING
        "created_at": datetime.now().isoformat()
    }
    db.save_clone_bot(bot_id, bot_data) # db.py ထဲက function ကို ခေါ်

    # 5. Notify Main Owner (ADMIN_ID)
    keyboard = [[
        InlineKeyboardButton("✅ Approve Clone", callback_data=f"clone_approve_{bot_id}"),
        InlineKeyboardButton("❌ Reject Clone", callback_data=f"clone_reject_{bot_id}")
    ]]
    reply_markup = InlineKeyboardMarkup(keyboard)
    ...
    try:
        await context.bot.send_message(
            chat_id=ADMIN_ID,
            text=f"🤖 ***Clone Bot Request***\n\n"
                 f"👤 **Requester:** [{user_name}](tg://user?id={user_id})\n"
                 f"🆔 **Requester ID:** `{user_id}`\n\n"
                 f"**New Bot Details:**\n"
                 f"🤖 **Bot:** @{bot_username}\n"
                 f"🆔 **Bot ID:** `{bot_id}`\n\n"
                 f"ဤ bot ကို clone အဖြစ် အသုံးပြုခွင့် ပေးမလား?",
            parse_mode="Markdown",
            reply_markup=reply_markup
        )
    except Exception as e:
        print(f"Failed to send clone request to ADMIN_ID: {e}")

    # 6. Reply to user
    await processing_msg.edit_text(
        f"✅ **Bot Request ပို့ပြီးပါပြီ!**\n\n"
        f"Bot: @{bot_username}\n\n"
        f"သင့် bot အတွက် တောင်းဆိုမှုကို Owner (Admin) ထံ ပို့လိုက်ပါပြီ။\n"
        f"Owner က approve လုပ်တာနဲ့ သင့် bot စတင် အလုပ်လုပ်ပါလိမ့်မယ်။"
    )
...
async def addbot_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    args = context.args
    if len(args) != 1:
        await update.message.reply_text("❌ Format: /addbot <bot_token>")
        return

    bot_token = args[0]
    try:
        await update.message.reply_text("⏳ Token ကို စစ်ဆေးနေပါသည်...")
        temp_bot = Bot(token=bot_token)
        bot_info = await temp_bot.get_me()
        bot_username = bot_info.username
        bot_id = str(bot_info.id)

        if db.get_clone_bot(bot_id): # Active or Pending ရှိမရှိ စစ်
        ...
        bot_data = {
            ...
            "owner_id": user_id, 
            "balance": 0,
            "status": "active", # <--- Admin က ထည့်လျှင် တန်း Active ဖြစ်သည်
...
async def removebot_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    args = context.args
    if len(args) != 1:
        await update.message.reply_text("❌ မှန်ကန်တဲ့ format: /removebot <bot_id>", parse_mode="Markdown")
        return
    ...
    if remove_clone_bot(bot_id):
        ...
        await update.message.reply_text(f"❌ Bot ID `{bot_id}` မတွေ့ပါ!", parse_mode="Markdown")
...
async def addfund_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    bot_found = db.get_clone_bot_by_admin(admin_id)
    if not bot_found:
        await update.message.reply_text(f"❌ Admin ID `{admin_id}` နဲ့ bot မတွေ့ပါ!", parse_mode="Markdown")
        return

    bot_id_found = str(bot_found.get("_id"))
    db.update_clone_bot_balance(bot_id_found, amount)
    new_balance = bot_found.get("balance", 0) + amount
...
async def clone_bot_mmb(update: Update, context: ContextTypes.DEFAULT_TYPE, bot_id, admin_id):
    ...
    if len(args) != 3:
        await update.message.reply_text("❌ မှန်ကန်တဲ့ format: /mmb gameid serverid amount")
        return
    ...
    price = get_price(diamonds) # Uses DB-backed prices
    if not price:
        await update.message.reply_text(f"❌ {diamonds} diamonds မရရှိနိုင်ပါ!")
        return

    order_data = {
        "bot_id": bot_id, "user_id": user_id, "username": user.username or user.first_name,
        "game_id": game_id, "server_id": server_id, "diamonds": diamonds, "price": price,
...
async def clone_bot_callback(update: Update, context: ContextTypes.DEFAULT_TYPE, bot_id, admin_id):
    ...
    if data.startswith("clone_accept_"):
        ...
        try:
            await context.bot.send_message(
                chat_id=ADMIN_ID, # Send to MAIN OWNER
                text=(
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            for group_id in ADMIN_GROUP_IDS:
                try:
                    ...
                    print(f"Failed to send register notification to group {group_id}: {loop_e}")
        except Exception as e:
            print(f"Error preparing group notification for register approve: {e}")
            pass
            ...
            try:
                ...
                for group_id in ADMIN_GROUP_IDS:
                    try:
                        ...
                        print(f"Failed to send topup approve notification to group {group_id}: {loop_e}")
            except Exception as e:
                print(f"Error preparing group notification for topup approve: {e}")
                pass
            ...
            try:
                ...
                for group_id in ADMIN_GROUP_IDS:
                    try:
                        ...
                        print(f"Failed to send topup reject notification to group {group_id}: {loop_e}")
            except Exception as e:
                print(f"Error preparing group notification for topup reject: {e}")
                pass
                ...
                for group_id in ADMIN_GROUP_IDS:
                    try:
                        if await is_bot_admin_in_group(context.bot, group_id):
                         ...
                         print(f"Failed to send order confirm notification to group {group_id}: {loop_e}")
            except Exception as e:
                print(f"Error preparing group notification for order confirm: {e}")
                pass
            ...
            try:
                ...
                for group_id in ADMIN_GROUP_IDS:
                    try:
                        ...
                        print(f"Failed to send order cancel notification to group {group_id}: {loop_e}")
            except Exception as e:
                print(f"Error preparing group notification for order cancel: {e}")
                pass
    ...
    # --- (NEW) Clone Bot Approval Logic ---
    elif query.data.startswith("clone_approve_"):
        if not is_owner(user_id): # Only main owner can approve
            await query.answer("❌ Owner သာ clone bot များကို approve လုပ်နိုင်ပါသည်။", show_alert=True)
            return
            
        bot_id = query.data.replace("clone_approve_", "")
        bot_data = db.get_clone_bot(bot_id)
        
        if not bot_data:
            await query.edit_message_text(f"{query.message.text}\n\n❌ Bot data မတွေ့ပါ (DB Error)။")
            return
        
        if bot_data.get("status") == "active":
            await query.answer("ℹ️ ဒီ bot က active ဖြစ်ပြီးသားပါ။", show_alert=True)
            return
            
        # 1. Update status
        db.update_clone_bot_status(bot_id, "active")
        
        # 2. Start the bot
        bot_token = bot_data.get("token")
        requester_id = bot_data.get("owner_id")
        if bot_token and requester_id:
            asyncio.create_task(run_clone_bot(bot_token, bot_id, requester_id))
            
        # 3. Edit owner message
        await query.edit_message_text(
            f"{query.message.text}\n\n"
            f"✅ **Clone Bot Approved** by {admin_name}."
        )
        ...
        try:
            await context.bot.send_message(
                ...
                text=f"🎉 ***Bot Approved!***\n\n"
                     f"သင်တောင်းဆိုထားသော bot (@{bot_data.get('username')}) ကို Owner မှ လက်ခံလိုက်ပါပြီ။\n"
                     f"Bot သည် ယခု စတင် အလုပ်လုပ်နေပါပြီ။"
            )
        except Exception as e:
            print(f"Failed to notify requester {requester_id}: {e}")
        
        await query.answer("✅ Bot Approved and Started!")
        return

    elif query.data.startswith("clone_reject_"):
        if not is_owner(user_id):
            await query.answer("❌ Owner သာ clone bot များကို reject လုပ်နိုင်ပါသည်။", show_alert=True)
            return

        bot_id = query.data.replace("clone_reject_", "")
        bot_data = db.get_clone_bot(bot_id)
        ...
        # 1. Delete from DB
        db.remove_clone_bot(bot_id)
        
        # 2. Edit owner message
        await query.edit_message_text(
            f"{query.message.text}\n\n"
            f"❌ **Clone Bot REJECTED** by {admin_name}."
        )
        
        # 3. Notify requester
        try:
            await context.bot.send_message(
                chat_id=bot_data.get("owner_id"),
                text=f"🚫 ***Bot Rejected***\n\n"
                     f"သင်တောင်းဆိုထားသော bot (@{bot_data.get('username')}) ကို Owner မှ ငြင်းပယ်လိုက်ပါသည်။"
            )
        except Exception as e:
            print(f"Failed to notify requester {bot_data.get('owner_id')}: {e}")
            
        await query.answer("❌ Bot Rejected and Deleted.")
        return
    # --- (End of Clone Bot Approval Logic) ---
    ...
    # Handle main owner approve/reject clone bot orders
        ...
        await query.answer("❌ ဤ function ကို /smileorder (API) command သို့ ပြောင်းရွှေ့ထားပါသည်။")
        return
...
async def post_init(application: Application):
    """Called after application initialization - start ACTIVE clone bots"""
    
    # Load ONLY active bots on startup
    clone_bots = db.load_clone_bots(active_only=True) # <--- အရေးကြီးသော ပြောင်းလဲမှု
    
    for bot_id, bot_data in clone_bots.items():
        bot_token = bot_data.get("token")
        admin_id = bot_data.get("owner_id") # This is the clone owner
        if bot_token and admin_id:
            asyncio.create_task(run_clone_bot(bot_token, bot_id, admin_id))
            print(f"🔄 Starting ACTIVE clone bot {bot_id}...")
    
    # (Smile.one code မပါတော့)
...
def main():
    ...
    load_global_settings()
    load_authorized_users() # <--- သုံးခွင့်ရှိသူစာရင်းကို အရင် load လုပ်ပါ
    load_admin_ids_global()
    ...
    try:
        ...
        # User ရှိပြီးသား, ဒါပေမယ့် balance 0 ဖြစ်နေပြီး, activity (order/topup) မရှိသေးရင်
            ...
            # User က ရှိပြီးသား၊ ပိုက်ဆံလည်း ရှိနေရင် ဘာမှမလုပ်တော့ပါ
            print(f"User {target_user_id} already has balance or activity. No changes made to balance.")
            
        # --- 2. Authorization Check (အသစ်ထပ်ထည့်ထား) ---
        ...
        if target_user_id not in AUTHORIZED_USERS:
            ...
            db.add_authorized_user(target_user_id)
            load_authorized_users() # Global set (AUTHORIZED_USERS) ကို DB မှ ပြန် reload လုပ်ပါ
    ...
    application.add_handler(CommandHandler("clonetopupbot", clonetopupbot_command))
